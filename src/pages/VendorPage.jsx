...
const VendorPage = ({ type }) => {
  ...
  const { vendors, status, selectedVendor } = useSelector(
    (state) => state.vendor
  );
  ...
  const categories = [
    ...
    "SECURITY",
    "CATERING",
    "FLOWER_AND_DECORATION",
    "PHOTOGRAPHY_AND_VIDEOGRAPHY",
    "TECHNOLOGY_AND_MULTIMEDIA",
    "ENTERTAINER",
  ];
  ...
  const filteredVendors = useMemo(() => {
    return vendors.filter((vendor) => {
      ...
      const matchesCategory =
        selectedCategory === "ALL" || vendor.mainCategory === selectedCategory;
      const matchesSearch =
        searchQuery.trim() === "" ||
  ...
  }, [vendors, selectedStatus, selectedCategory, searchQuery, type]);
  ...
  return (
    <div className="container mx-auto h-screen pt-20 pb-20">
      ...
      <div className="relative w-full flex justify-center mb-10 space-x-4 items-center">
        <div className="relative w-1/2 group">
          <div className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none">
            <FaSearch className="h-6 w-6 text-gray-400 group-focus-within:text-[#00AA55] transition-colors duration-300" />
          </div>
          <input
            type="text"
